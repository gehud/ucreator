...
/// The path to the `Cargo.toml` file for the UCreator project.
pub struct Manifest {
...
impl Manifest {
    ...
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        ...
        let find_in_deps = |deps: &Item| -> Option<syn::Path> {
            ...
            } else if let Some(dep) = deps.get(UENGINE) {
                dep_package(dep).unwrap_or(UENGINE)
            } else {
                return None;
            };

            let mut path = Self::parse_str::<syn::Path>(package);
            if let Some(module) = name.strip_prefix("uengine_") {
    ...
    pub fn get_subcrate(&self, subcrate: &str) -> Option<syn::Path> {
        self.maybe_get_path(UENGINE)
            ...
            .or_else(|| self.maybe_get_path(&format!("uengine_{subcrate}")))
    }
}
